...
Traducido al español el 14/01/2020 (Con google traductor)
...
; [F3+T] [Tecla Esc: menú Juego → Configuración → Paquete de recursos → Listo]
...
; Nombre mostrado 
* No utilice caracteres de doble byte, solo caracteres y símbolos alfanuméricos de byte único

AddDisplayName = ja_JP, AH-6 Killer egg
; Nombre mostrado al sostener el artículo en idioma japonés
...
; Agregar un asiento 
; ※ Debe haber al menos un asiento excepto UAV
...
;   helicopter	: todos los helicópteros
;   plane	: Todos los aviones de ala fija
;   vehicle	: Todas las armas terrestres
...
; true, 	2 → Disponible para el 2º jugador. El piloto se puede usar cuando no hay un jugador en el segundo asiento.
; false, 	2 → Disponible para el 2º jugador. Piloto no disponible.
; false, 	1 → Solo se pueden usar pilotos (no recomendado)
; true, 	1 → piloto solo disponible
; true,	  1 si se omite.
;
...
AddPartWeaponChild   = falso, 	   true, 0.00, 0.5, 3.00
...
AddPartLG =         Pos X, Pos Y, Pos Z,     ER  X, ER  Y, ER  Z,   θ                       [, ER  X, ER  Y, ER  Z,   θ]
AddPartLGRev =      Pos X, Pos Y, Pos Z,     ER  X, ER  Y, ER  Z,   θ                       [, ER  X, ER  Y, ER  Z,   θ]
AddPartSlideRotLG = CM  X, CM  Y, CM  Z,     Pos X, Pos Y, Pos Z,   ER  X, ER  Y, ER  Z,    θ
AddPartLGHatch =    Pos X, Pos Y, Pos Z,     ER  X, ER  Y, ER  Z,   θ                       [, ER  X, ER  Y, ER  Z,   θ]
; Pos: posición / coordenada
; CM:  Cantidad de movimiento
; ER:  Eje de rotación
; θ:   Ángulo de rotación [0;180]
...
;   Nota: false cuando el punto de rotación de la pista se establece en el sentido de las agujas del reloj como se ve desde el lado izquierdo del tanque, y verdadero cuando lo contrario es cierto.
; Funciona sin ruedas.
...
; Si es false, prohibir el movimiento y la rotación en el suelo
...
; Si cambiar el modo artillero (true = sí, false = no)

EnableNightVision = true
; Si cambiar la visión nocturna (true = sí, false = no)

EnableEntityRadar = false
; Presencia de radar (true = sí, false = no)
...
;Coordenada de lanzamiento de la bengalas
...
; Si es true, habilite el sombreado suave para mostrar las esquinas suavemente
